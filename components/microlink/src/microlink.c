...
microlink_t *microlink_init(const microlink_config_t *config) {
    ...
    if (ml->config.control_host && ml->config.control_host[0] != '\0') {
        strncpy(ml->ctrl_host, ml->config.control_host, sizeof(ml->ctrl_host) - 1);
        ml->ctrl_host[sizeof(ml->ctrl_host) - 1] = '\0';
        ESP_LOGI(TAG, "Control plane from config: %s", ml->ctrl_host);
    }

    /* Copy advertise routes for subnet routing */
    if (ml->config.advertise_routes && ml->config.advertise_routes[0] != '\0') {
        strncpy(ml->advertise_routes, ml->config.advertise_routes, sizeof(ml->advertise_routes) - 1);
        ml->advertise_routes[sizeof(ml->advertise_routes) - 1] = '\0';
        ml->subnet_route_state = ML_SUBNET_ROUTE_UNKNOWN;
        ESP_LOGI(TAG, "Advertise routes: %s", ml->advertise_routes);
    } else {
        ml->advertise_routes[0] = '\0';
        ml->subnet_route_state = ML_SUBNET_ROUTE_NOT_ADVERTISED;
    }
    ml->enable_subnet_routing = ml->config.enable_subnet_routing;
...
static esp_err_t microlink_set_wg_napt_api(void *arg)
{
    ...
    if (!ctx || !ctx->netif || (ctx->enable && !netif_is_up(ctx->netif))) {
        return ESP_ERR_INVALID_STATE;
    }

    if (ctx->enable) {
        for (struct netif *n = netif_list; n != NULL; n = n->next) {
            if (n != ctx->netif && n->napt == 1) {
                ESP_LOGW(TAG, "Disabling pre-existing NAPT on %c%c%d before enabling WireGuard NAPT",
                         n->name[0], n->name[1], n->num);
                ip_napt_enable_netif(n, 0);
            }
        }
    }

    int ret = ip_napt_enable_netif(ctx->netif, ctx->enable ? 1 : 0);
    ESP_LOGI(TAG, "ip_napt_enable_netif(%c%c%d,%d) ret=%d now_napt=%d flags=0x%02x",
             ctx->netif->name[0], ctx->netif->name[1], ctx->netif->num,
             ctx->enable ? 1 : 0, ret, ctx->netif->napt, ctx->netif->flags);
    return ret ? ESP_OK : ESP_FAIL;
}

static void format_ip4_addr(const ip4_addr_t *ip, char *buf, size_t len)
{
    if (!buf || len == 0) return;
    if (!ip) {
...
static bool parse_first_advertised_cidr_host(const char *routes, ip4_addr_t *out)
{
    unsigned a, b, c, d, prefix;
    if (!routes || !out) return false;
    const char *p = routes;
    while (*p && (*p < '0' || *p > '9')) p++;
    if (sscanf(p, "%u.%u.%u.%u/%u", &a, &b, &c, &d, &prefix) != 5) return false;
    ...
    if (prefix < 32) {
        uint32_t host_mask = (1UL << (32 - prefix)) - 1UL;
        if (host_mask > 1 && (h & host_mask) == 0) {
            h += 1; /* first usable host; for 192.168.24.0/24 this is .1 */
        }
    }
    IP4_ADDR(out, (h >> 24) & 0xFF, (h >> 16) & 0xFF, (h >> 8) & 0xFF, h & 0xFF);
    return true;
}

static void log_route_to_dest(const char *label, const ip4_addr_t *src, const ip4_addr_t *dest)
{
    char src_s[16], dst_s[16], out_ip[16], out_mask[16], out_gw[16];
    format_ip4_addr(src, src_s, sizeof(src_s));
    format_ip4_addr(dest, dst_s, sizeof(dst_s));

    struct netif *out = ip4_route_src(src, dest);
    if (!out) {
        ESP_LOGW(TAG, "GW DIAG route %-14s src=%s dst=%s -> NO_ROUTE", label, src_s, dst_s);
        return;
    }

    format_ip4_addr(netif_ip4_addr(out), out_ip, sizeof(out_ip));
    format_ip4_addr(netif_ip4_netmask(out), out_mask, sizeof(out_mask));
    format_ip4_addr(netif_ip4_gw(out), out_gw, sizeof(out_gw));
    ESP_LOGI(TAG, "GW DIAG route %-14s src=%s dst=%s -> %c%c%d ip=%s mask=%s gw=%s napt=%d flags=0x%02x",
             label, src_s, dst_s, out->name[0], out->name[1], out->num,
             out_ip, out_mask, out_gw, out->napt, out->flags);
}

static esp_err_t microlink_log_gateway_diagnostics_api(void *arg)
{
    gateway_diag_ctx_t *ctx = (gateway_diag_ctx_t *)arg;
    microlink_t *ml = ctx ? ctx->ml : NULL;
    const char *reason = (ctx && ctx->reason) ? ctx->reason : "manual";

    ESP_LOGW(TAG, "=== GW DIAG START reason=%s ===", reason);
    if (ml) {
        ESP_LOGW(TAG, "GW DIAG ml=%p state=%d vpn_ip=%u.%u.%u.%u advertise_routes=%s route_state=%d wg_netif=%p",
                 ml, ml->state,
                 (unsigned)((ml->vpn_ip >> 24) & 0xFF),
                 (unsigned)((ml->vpn_ip >> 16) & 0xFF),
                 (unsigned)((ml->vpn_ip >> 8) & 0xFF),
                 (unsigned)(ml->vpn_ip & 0xFF),
                 ml->advertise_routes[0] ? ml->advertise_routes : "(none)",
                 (int)ml->subnet_route_state,
                 ml->wg_netif);
    }

    for (struct netif *n = netif_list; n != NULL; n = n->next) {
        char ip[16], mask[16], gw[16];
        format_ip4_addr(netif_ip4_addr(n), ip, sizeof(ip));
        format_ip4_addr(netif_ip4_netmask(n), mask, sizeof(mask));
        format_ip4_addr(netif_ip4_gw(n), gw, sizeof(gw));
        ESP_LOGW(TAG, "GW DIAG netif %c%c%d ptr=%p ip=%s mask=%s gw=%s flags=0x%02x up=%d link=%d napt=%d mtu=%u input=%p output=%p state=%p",
                 n->name[0], n->name[1], n->num, n, ip, mask, gw,
                 n->flags, netif_is_up(n), netif_is_link_up(n), n->napt,
                 n->mtu, n->input, n->output, n->state);
    }

    if (ml && ml->wg_netif) {
        struct netif *wg = (struct netif *)ml->wg_netif;
        ip4_addr_t src = *netif_ip4_addr(wg);
        ip4_addr_t lan_dest;
        if (parse_first_advertised_cidr_host(ml->advertise_routes, &lan_dest)) {
            log_route_to_dest("advertised", &src, &lan_dest);
        }

        esp_netif_t *sta = esp_netif_get_handle_from_ifkey("WIFI_STA_DEF");
        if (sta) {
            esp_netif_ip_info_t sta_ip;
            if (esp_netif_get_ip_info(sta, &sta_ip) == ESP_OK && sta_ip.gw.addr != 0) {
                ip4_addr_t sta_gw;
                ip4_addr_set_u32(&sta_gw, sta_ip.gw.addr);
                log_route_to_dest("sta_gateway", &src, &sta_gw);
            } else {
                ESP_LOGW(TAG, "GW DIAG STA esp_netif has no gateway yet");
            }
        } else {
            ESP_LOGW(TAG, "GW DIAG WIFI_STA_DEF esp_netif not found");
        }
    }

#if LWIP_STATS && IP_NAPT_STATS
    struct stats_ip_napt stats;
    ip_napt_get_stats(&stats);
    ESP_LOGW(TAG, "GW DIAG NAPT stats tcp=%lu udp=%lu icmp=%lu forced_evictions=%lu",
             (unsigned long)stats.nr_active_tcp,
             (unsigned long)stats.nr_active_udp,
             (unsigned long)stats.nr_active_icmp,
             (unsigned long)stats.nr_forced_evictions);
#else
    ESP_LOGW(TAG, "GW DIAG NAPT stats unavailable (CONFIG_LWIP_STATS=%d IP_NAPT_STATS=%d)",
             (int)LWIP_STATS, (int)IP_NAPT_STATS);
#endif

    ESP_LOGW(TAG, "=== GW DIAG END reason=%s ===", reason);
    return ESP_OK;
}

void microlink_log_gateway_diagnostics(microlink_t *ml, const char *reason)
{
    gateway_diag_ctx_t ctx = {
        .ml = ml,
        .reason = reason,
    };
    esp_err_t ret = esp_netif_tcpip_exec(microlink_log_gateway_diagnostics_api, &ctx);
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "Gateway diagnostics failed: %s", esp_err_to_name(ret));
    }
}
...
esp_err_t microlink_set_wg_napt(microlink_t *ml, bool enable)
{
    ...
    esp_err_t ret = esp_netif_tcpip_exec(microlink_set_wg_napt_api, &ctx);
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "WireGuard NAPT %s", enable ? "enabled" : "disabled");
        microlink_log_gateway_diagnostics(ml, enable ? "wg_napt_enabled" : "wg_napt_disabled");
    } else {
        ESP_LOGE(TAG, "WireGuard NAPT %s failed: %s",
...
microlink_subnet_route_state_t microlink_get_subnet_route_state(const microlink_t *ml) {
    return ml ? ml->subnet_route_state : ML_SUBNET_ROUTE_NOT_ADVERTISED;
}
