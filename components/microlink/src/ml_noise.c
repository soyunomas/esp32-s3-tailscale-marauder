...
static int chacha20poly1305_encrypt(const uint8_t *key, uint64_t nonce,
    ...
    const uint8_t *pt = plaintext ? plaintext : &empty;
    size_t out_len = 0;
    status = psa_aead_encrypt(key_id, PSA_ALG_CHACHA20_POLY1305,
                              nonce_bytes, sizeof(nonce_bytes),
                              ad, ad_len, pt, pt_len,
                              ciphertext, pt_len + 16, &out_len);

    psa_destroy_key(key_id);
    return (status == PSA_SUCCESS && out_len == pt_len + 16) ? 0 : -1;
}
...
static int chacha20poly1305_decrypt(const uint8_t *key, uint64_t nonce,
    ...
    uint8_t empty;
    uint8_t *pt = plaintext ? plaintext : &empty;
    size_t out_len = 0;
    (void)tag;
    status = psa_aead_decrypt(key_id, PSA_ALG_CHACHA20_POLY1305,
                              ...
                              ad, ad_len,
                              ciphertext, ct_len,
                              pt, payload_len, &out_len);

    psa_destroy_key(key_id);
    return (status == PSA_SUCCESS && out_len == payload_len) ? 0 : -1;
}
...
esp_err_t ml_noise_write_msg1(ml_noise_state_t *state, uint8_t *out, size_t *out_len) {
    ...
    out[pos++] = ML_CTRL_PROTOCOL_VER & 0xFF;          /* version low (138 = 0x8a) */
