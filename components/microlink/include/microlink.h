...
typedef struct {
    ...
    const char *control_host;   /* Control plane hostname, NULL/empty = Tailscale */
    ...
    /* Subnet routing */
    const char *advertise_routes;    /* JSON array string, e.g. '["192.168.24.0/24"]' or NULL */
    bool enable_subnet_routing;      /* Reserved for future SNAT/forwarding; route ads use advertise_routes */
...
typedef enum {
    ML_SUBNET_ROUTE_UNKNOWN = 0,        /* Route advertised, no authoritative map data yet */
    ML_SUBNET_ROUTE_NOT_ADVERTISED,     /* No route configured for this node */
    ML_SUBNET_ROUTE_PENDING,            /* Advertised, but not present in AllowedIPs/PrimaryRoutes */
    ML_SUBNET_ROUTE_APPROVED,           /* Present in AllowedIPs but not primary */
    ML_SUBNET_ROUTE_ACTIVE,             /* Present in PrimaryRoutes */
} microlink_subnet_route_state_t;
...
/**
 * @brief Enable/disable lwIP NAPT on the WireGuard interface.
 *
 * Gateway/SNAT mode needs NAPT on the incoming Tailscale/WireGuard netif so
 * packets forwarded to the LAN leave with the ESP32 STA address as source.
 */
esp_err_t microlink_set_wg_napt(microlink_t *ml, bool enable);

/**
 * @brief Log current WireGuard/LAN gateway diagnostics.
 *
 * Intended for debugging subnet routing. Logs netif addresses, flags, NAPT
 * state, selected route toward the LAN gateway, and optional NAPT stats.
 */
...
/**
 * @brief Get subnet route approval state inferred from self Node in MapResponse.
 */
