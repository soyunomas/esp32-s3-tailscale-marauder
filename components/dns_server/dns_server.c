...
static int forward_dns_request(const char *req, size_t req_len, char *reply, size_t reply_max_len)
{
    const ip_addr_t *dns = dns_getserver(0);
    if (!dns || !IP_IS_V4(dns) || ip4_addr_isany(ip_2_ip4(dns))) {
        return -1;
    }
    ...
    dest.sin_addr.s_addr = ip4_addr_get_u32(ip_2_ip4(dns));

    int ret = sendto(sock, req, req_len, 0, (struct sockaddr *)&dest, sizeof(dest));
    if (ret < 0) {
        close(sock);
        return -1;
    }

    ret = recv(sock, reply, reply_max_len, 0);
    close(sock);
    return ret;
}
    ...
    while (handle->started) {
        ...
        while (handle->started) {
            ...
            // Data received
                ...
                int reply_len = handle->forward
                    ? forward_dns_request(rx_buffer, len, reply, DNS_MAX_LEN)
...
void dns_server_set_forwarding(dns_server_handle_t handle, bool enabled)
{
    if (handle) {
        handle->forward = enabled;
    }
}
